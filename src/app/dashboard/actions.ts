...
export async function getPropertyById(
  ...
  try {
    ...
    logSecurityEvent(
      'property_found',
      { id: sanitizedId, name: property.name },
      'info',
    );
...
export async function updatePropertyStatus(
  ...
  try {
    ...
    if (!idValidation.valid) {
      logSecurityEvent(
        'invalid_property_id_update',
        { id, error: idValidation.error },
        'warn',
      );
      return {
        data: null,
    ...
    if (!statusValidation.valid) {
      logSecurityEvent(
        ...
        { id, status, error: statusValidation.error },
        'warn',
      );
      return {
        data: null,
    ...
    logSecurityEvent(
      'property_status_update_attempt',
      {
        id: sanitizedId,
 ...
 */
export async function getPropertySummary(): Promise<
...
> {
  try {
    ...
    const summary: PropertySummary = {
      ...
      completeProperties: properties.data.filter((p: Property) => p.status === 'complete')
        .length,
      pendingProperties: properties.data.filter((p: Property) => p.status === 'pending')
        .length,
      overdueProperties: properties.data.filter((p: Property) => p.status === 'overdue')
        .length,
      totalUnits: properties.data.reduce((sum: number, p: Property) => sum + p.units, 0),
      totalTasks: properties.data.reduce(
        (sum: number, p: Property) => sum + p.tasks.completed + p.tasks.pending + p.tasks.overdue,
        0,
      ),
      completedTasks: properties.data.reduce(
        (sum: number, p: Property) => sum + p.tasks.completed,
        0,
      ),

...
export function PropertyCard({ property }: PropertyCardProps) {
  ...
  const handleStatusUpdate = (newStatus: PropertyStatus) => {
    ...
    startTransition(async () => {
      try {
        const response = await updatePropertyStatus(currentProperty.id, newStatus);
    ...
    >
      ...
      <CardContent className="pt-0 space-y-4">
        ...
        <div className="p-3 rounded-lg border bg-muted/30">
          <ProgressBar 
            value={currentProperty.maintenanceScore} 
            size="md"
            className="w-full"
          />
        </div>
        ...
        <div className="grid grid-cols-3 gap-2" role="group" aria-label="Task breakdown">
          <div className="p-3 bg-emerald-50 dark:bg-emerald-950/20 rounded-lg text-center border border-emerald-200/50 dark:border-emerald-800/30">
            ...
            <p className="font-semibold text-emerald-600 dark:text-emerald-400 text-lg">{currentProperty.tasks.completed}</p>
          </div>
          <div className="p-3 bg-amber-50 dark:bg-amber-950/20 rounded-lg text-center border border-amber-200/50 dark:border-amber-800/30">
            ...
            <p className="font-semibold text-amber-600 dark:text-amber-400 text-lg">{currentProperty.tasks.pending}</p>
          </div>
          <div className="p-3 bg-red-50 dark:bg-red-950/20 rounded-lg text-center border border-red-200/50 dark:border-red-800/30">
            ...
            <p className="font-semibold text-red-600 dark:text-red-400 text-lg">{currentProperty.tasks.overdue}</p>
          </div>
        </div>
        ...
        {currentProperty.issues.length > 0 && (
          <div>
            ...
            <div className="space-y-1">
              {currentProperty.issues.map((issue, index) => (
                <div 
                  key={index} 
